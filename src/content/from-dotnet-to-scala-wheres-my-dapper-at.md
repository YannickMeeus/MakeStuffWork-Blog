...
From an in-memory cache to a globally distributed data model, pretty much every solution has storage requirements.
Scala seems to encompass a wide range of paradigms. Some use it best in a Java++ way,
while others try to approach it with Category Theory front and centre of their endeavours.

This means that “idiomatic” database access code depends on what idioms you embrace.

For now, I will stay on the ‘safe’ end of the spectrum and have a crack
...
I alias the above snippet to `dbpg` in my bash profile. If you’re
...
is made available to you when the container is up and running.

From here on out, I’ll be interacting with my database entirely from within the
application. This includes creating a few tables and performing some CRUD operations.

As always, I’ll start off with the [g8 seed template](https://github.com/scala/scala-seed.g8),
and worked my way up from there.

I will use [scalikejdbc](http://scalikejdbc.org/) as the library of choice.
...
So we have ourselves a database, we have ourselves a library, let’s write us some code!

We’ll need a few dependencies declared in our build.sbt file. As a reminder,
...
So we’ve made a start on at least the plumbing required. I’m importing the driver
...
said contract against a data source. This has several benefits:
...
followed by `apply` will evaluate the SQL statement, the `Try` will catch any exceptions if one occurs and the caller will have to deal with this accordingly.

Speaking of the caller, we can now change `EntryPoint` to include:
...
It’s rather self-explanatory, the pattern matching might throw first-time scala users off but essentially it’s a `switch` statement on steroids.

Now we have a table, we need to get some data in there. I’ll use a case class to represent our data,
...
The same pattern applies, but we’re using first converting the `Author` into a `DBAuthor`, and then use string interpolation to build up the SQL statement.
...
We’re noticing a pattern here and
most of the interactions are easy to model as long as
we’re working against a single table. I will not explore
...
And that’s that. I’ve created an Authors table, inserted data and
...
If you have any questions or comments, please reach out on Twitter.